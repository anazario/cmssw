...
double VertexHelper::CalculateCMCosTheta(const reco::Vertex &vertex, const reco::Track &track) {
  ...
  math::XYZTLorentzVector total;
  for(const auto &trackRef : vertex.tracks()) {
    const LorentzVec v(trackRef->pt(), trackRef->eta(), trackRef->phi(), 0.);
    total += math::XYZTLorentzVector(v.px(), v.py(), v.pz(), v.energy());
  }

  const LorentzVec tLV(track.pt(), track.eta(), track.phi(), 0.);
  math::XYZTLorentzVector trackVec(tLV.px(), tLV.py(), tLV.pz(), tLV.energy());

  const ROOT::Math::Boost boost(total.BoostToCM());
  trackVec = boost(trackVec);

  const math::XYZVector boostDir(total.px(), total.py(), total.pz());
  if (boostDir.r() < 1e-10 || trackVec.P() < 1e-10) return 0.;
  return math::XYZVector(trackVec.px(), trackVec.py(), trackVec.pz()).Unit().Dot(boostDir.Unit());
}
...
double VertexHelper::CalculateDecayAngle(const reco::Vertex &vertex) {
  ...
  math::XYZTLorentzVector total;
  math::XYZTLorentzVector firstTrackVec;
  bool first = true;

  for(const auto &track : vertex.tracks()) {
    const LorentzVec v(track->pt(), track->eta(), track->phi(), 0.);
    const math::XYZTLorentzVector v4(v.px(), v.py(), v.pz(), v.energy());
    total += v4;
    if (first) { firstTrackVec = v4; first = false; }
  }

  const ROOT::Math::Boost boost(total.BoostToCM());
  firstTrackVec = boost(firstTrackVec);

  const math::XYZVector boostDir(total.px(), total.py(), total.pz());
  if (boostDir.r() < 1e-10 || firstTrackVec.P() < 1e-10) return 0.;
  return math::XYZVector(firstTrackVec.px(), firstTrackVec.py(), firstTrackVec.pz()).Unit().Dot(boostDir.Unit());
}
...
std::pair<reco::TrackCollection, reco::TrackCollection> VertexHelper::FindMinMassHemispheres(const reco::Vertex &vertex) {
  ...
  using V4 = math::XYZTLorentzVector;
  using V3 = math::XYZVector;

  V4 total;
  std::vector<V4> track4Vecs;
  std::vector<const reco::Track*> trackPtrs;

  for (const auto &track : vertex.tracks()) {
    const LorentzVec v(track->pt(), track->eta(), track->phi(), 0.);
    const V4 v4(v.px(), v.py(), v.pz(), v.energy());
    total += v4;
    track4Vecs.emplace_back(v4);
    trackPtrs.emplace_back(track.get());
  }

  // Boost all tracks to the CM frame
  const ROOT::Math::Boost boost(total.BoostToCM());
  for (V4 &vec : track4Vecs)
    vec = boost(vec);

  reco::TrackCollection bestHem1, bestHem2;
  double bestMetric = -1;
  const int N = static_cast<int>(track4Vecs.size());

  for (int i = 0; i < N - 1; ++i) {
    for (int j = i + 1; j < N; ++j) {

      const V3 pi(track4Vecs[i].px(), track4Vecs[i].py(), track4Vecs[i].pz());
      const V3 pj(track4Vecs[j].px(), track4Vecs[j].py(), track4Vecs[j].pz());
      const V3 refNormal = pi.Cross(pj);
      ...
      for (int k = 0; k < N; ++k) {
        if (k == i || k == j) continue;
        const V3 pk(track4Vecs[k].px(), track4Vecs[k].py(), track4Vecs[k].pz());
        if (pk.Dot(refNormal) > 0) { hem1.push_back(*trackPtrs[k]); hemVec1 += track4Vecs[k]; }
        else                       { hem2.push_back(*trackPtrs[k]); hemVec2 += track4Vecs[k]; }
      }
      ...
      for (int h1 = 0; h1 < 2; ++h1) {
        for (int h2 = 0; h2 < 2; ++h2) {
          ...
          V4 tmpVec[2] = {hemVec1, hemVec2};

          tmpHem[h1].emplace_back(*trackPtrs[i]); tmpVec[h1] += track4Vecs[i];
          tmpHem[h2].emplace_back(*trackPtrs[j]); tmpVec[h2] += track4Vecs[j];

          const double metric = tmpVec[0].P() + tmpVec[1].P();
...
double VertexHelper::CalculateHadronicDecayAngle(const reco::Vertex &vertex) {
  ...
  const LorentzVec vertex4Vec = GetTracks4Vector(GetTracks(vertex));
  const LorentzVec hem4Vec1   = GetTracks4Vector(hemispheres.first);

  math::XYZTLorentzVector total(vertex4Vec.px(), vertex4Vec.py(), vertex4Vec.pz(), vertex4Vec.energy());
  math::XYZTLorentzVector hem(hem4Vec1.px(), hem4Vec1.py(), hem4Vec1.pz(), hem4Vec1.energy());

  const ROOT::Math::Boost boost(total.BoostToCM());
  hem = boost(hem);

  const math::XYZVector boostDir(total.px(), total.py(), total.pz());
  if (boostDir.r() < 1e-10 || hem.P() < 1e-10) return 0.;
  return math::XYZVector(hem.px(), hem.py(), hem.pz()).Unit().Dot(boostDir.Unit());
}
...
LorentzVec VertexHelper::GetTracks4Vector(const reco::TrackCollection &tracks) {

  LorentzVec tracks4Vec;
  for(const auto &track : tracks)
    tracks4Vec += LorentzVec(track.pt(), track.eta(), track.phi(), 0.);
  return tracks4Vec;
